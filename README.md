...
| Web Search | Tavily search tool          | Retrieves three relevant web search results based on the query.            |
| RAG        | Pinecone / nomic-embed-text | Retrieves relevant, pre-chunked document segments using cosine similarity. |
| Reranker   | Cohere rerank-v3.5          | Ranks document chunks to select the top five most relevant.                |
| Chatbot    | Gemma3:12b                  | Engages users with a friendly persona and closed-domain prompts.           |
...
* The LLM creates a search query from the text extracted by OCR and the original user question.
...
* The LLM is prompted with *persona*, *return format*, and *chatting with search results* as a system message.
...
* The LLM generates five different queries that are semantically similar to the user's question and uses them to search for five documents per query.
...
* The LLM is prompted with *persona*, *return format*, and *chatting with RAG* as a system message.
...
* The LLM is prompted with *persona*, *return format*, and *Chatting General* as a system message to respond to general conversations and chat history.
